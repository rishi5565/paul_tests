...
def img_to_text_aws_textract(b64_img, equation=False):

    if equation == False:
    
        s3 = boto3.client('s3')
        bucket_name="img-hold-server"
        ...
        r = get_math_ocr(b64_img)
        text = r.get('text')
        reformatted = split_equations_and_text(text)
        return {"ocr_data" : reformatted}
...
def img_to_text_multiple(api_key, b64_images_array):
    text_array = []
    for b64_img in b64_images_array:
        text_array.append(img_to_text_aws_textract(b64_img))
    
    total_transcript_data = "\n\n".join(text_array)

    summarized_transcript = summarize_langchain(api_key, total_transcript_data)

    return {"total_transcript" : total_transcript_data, "summarized_total_transcript" : summarized_transcript}
...
@app.route('/img2text_multiple', methods=['POST', 'GET'])
def img2text_multiple():

    if not os.path.exists("img2text_multiple_counter.txt"):
        with open("img2text_multiple_counter.txt", "w") as f:
            f.write("0")

    with open("img2text_multiple_counter.txt", "r") as f:
        counter = int(f.read())
    ...
    try:
        if (request.method == "POST"):
            api_key = request.get_json().get('api_key')
            b64_img_array = request.get_json().get('b64_img_array')

            r = img_to_text_multiple(api_key, b64_img_array)
            return r
        ...
        reference_number = generate_reference_number(8)
        logger.error(f"Error in /img2text_multiple route [Ref: {reference_number}]: \n{e}\n")
        return jsonify(f'Error occurred. Please check server log with Ref No. {reference_number}\nError Summary: {e}')       
